...
class PID_Controller:
    def __init__(self, kp, ki, kd, num_log, validate_ki):
        self.kp = kp
        self.ki = ki
        self.kd = kd
    ...
    def get_output(
        ...
        self.error.append(measured - target)
        self.integral += self.error[-1]
        self.derivative = self.error[-1] - self.error[-2]
        self.output = (
            self.kp * self.error[-1]
            + self.ki * self.integral * (self.count >= self.validate_ki)
    ...
    def reset(self):
        self.error = deque([0] * self.num_log, maxlen=self.num_log)
        self.integral = 0
        self.derivative = 0
        self.output = 0
        self.count = 0
...
def PID_adjust_direction2(controller, target_azimuth, magx_off, magy_off):
    """
    目標角度に合わせて方向調整を行う関数
...
def PID_run2(controller, target_azimuth, magx_off, magy_off, loop_num):
    """
    目標地点までの方位角が既知の場合にPID制御により走行する関数
    """
...
def drive(lon_dest, lat_dest, thd_distance, t_run, log_path, t_start=0):
    ...
    stop_controller = PID_Controller(
        kp=0.4,
        ki=0.03,
        kd=3,
        num_log=50,
    ...
    running_controller = PID_Controller(
        kp=0.25,
        ki=0.02,
        kd=5,
        num_log=25,
    ...
    while distance > thd_distance:
        ...
        time.sleep(9)
        running_controller.reset()  # PID制御のリセット
        ...
        while time.time() - t_cal <= t_run:
            ...
            direction = gps_navigate.vincenty_inverse(
                lat_now, lon_now, lat_dest, lon_dest
            )
...
if __name__ == "__main__":
    ...
    drive(
        ...
        thd_distance=5,
        t_run=60,
        log_path="/home/dendenmushi/cansat2023/sequence/log/gpsrunningLog.txt",
    )
